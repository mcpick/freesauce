...
export const POST: APIRoute = async ({ request }) => {
    ...
    try {
        ...
        const unverifiedShops = await db
            ...
            .from(shops)
            .where(and(eq(shops.verified, 0), isNull(shops.google_place_id)));
        ...
        for (const shop of unverifiedShops) {
            try {
                ...
                if (result.verified) {
                    ...
                    verifiedCount++;
                    console.log(
                        `✅ Verified shop ${shop.id}: ${shop.name} (confidence: ${result.confidence})`,
                    );
                } else {
                    ...
                    failedCount++;
                    console.log(
                        `❌ Could not verify shop ${shop.id}: ${shop.name} (confidence: ${result.confidence})`,
                    );
                }
            } catch (shopError) {
                failedCount++;
