    ...
    try {
        ...
        if (!lat || !lng || isNaN(lat) || isNaN(lng)) {
            ...
            if (!geoData || geoData.length === 0) {
                return new Response(
                    JSON.stringify({
                        error: "Couldn't find that address on the map. Double-check it?",
                    }),
                    { status: 400, headers: { 'Content-Type': 'application/json' } },
                );
            }
        ...
        if (photo && photo instanceof File) {
            try {
                ...
                await db.update(shops).set({ photo_key }).where(eq(shops.id, shopId));
            } catch (photoErr) {
        ...
        const verifyPromise = (async () => {
            try {
                ...
                if (result.verified) {
                    ...
                    console.log(
                        `Shop ${shopId} could not be verified: ${result.confidence} confidence`,
                    );
                }
            } catch (e) {
