...
await test('GET /api/shops', async () => {
    ...
    if (!Array.isArray(shops))
        throw new Error('Response not array: ' + JSON.stringify(shops).substring(0, 200));
...
await test('Client JS uses querySelector not form.name', async () => {
    const r = await fetch(`${BASE}/add`);
    const html = await r.text();
    if (html.includes('form.name.value'))
        throw new Error('Still using form.name.value - DOM property clash!');
